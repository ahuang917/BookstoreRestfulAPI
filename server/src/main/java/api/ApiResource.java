...
public class ApiResource {
    ...
    public List<Book> booksByCategoryName(@PathParam("category-name") String categoryName,
        ...
        try {
            ...
            if (category == null) {
                throw new ApiException.InvalidParameter(String.format("No such category name: %d", categoryName));
            }
            List<Book> result = bookDao.findByCategoryId(category.getCategoryId());
            if (result == null)
            {
                throw new ApiException(String.format("No books in category"));
            }
            return result;
        } catch (Exception e) {
